...
//
//
// Related Topics 数组 哈希表 计数 滑动窗口 模拟 👍 21 👎 0
...
class Solution {
    public int minArrivalsToDiscard(int[] arrivals, int w, int m) {
        ...
        for (int i = 0; i < arrivals.length; i++) {
            if (mp.getOrDefault(arrivals[i], 0) == m) {
                arrivals[i] = -1;
                ans ++;
            } else {
                mp.put(arrivals[i], mp.getOrDefault(arrivals[i], 0) + 1);
            }
            int l = i-w+1;
            if (l >= 0) mp.merge(arrivals[l], -1, Integer::sum);
        }
        return ans;
    }
}
